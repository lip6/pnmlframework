 ...
 * @author lom
 */
public class CoreModelStats implements PnmlDocStatistics {
	/**
	...
	private void processPages(PageHLAPI page) {
		for (PageHLAPI innerPages : page.getObjects_PageHLAPI()) {
			processPages(innerPages);
		}
		numPlaces += page.getObjects_PlaceHLAPI().size();
		numTransitions += page.getObjects_TransitionHLAPI().size();
		numArcs += page.getObjects_ArcHLAPI().size();
		numRefPlaces += page.getObjects_RefPlaceHLAPI().size();
		numRefTransitions += page.getObjects_RefTransitionHLAPI().size();
	}
